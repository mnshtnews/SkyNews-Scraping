...
Architecture:
  • Uses Playwright (headless Chromium) for JavaScript-rendered listing pages
  • Falls back to httpx for article detail pages (faster, no JS needed)
  • ETag / Last-Modified conditional GETs for smart polling
  • Batch article fetching with configurable concurrency
"""
...
# Selectors that confirm the listing page has loaded content
        ...
        if wait_for_content:
            for selector in _CONTENT_READY_SELECTORS:
                try:
                    ...
                    await asyncio.sleep(1.5)  # let lazy-loaded images populate
                    break
    ...
    async def _fetch_article_detail(self, stub: RawArticle) -> RawArticle:
        ...
        try:
            with open("article_debug.html", "w", encoding="utf-8") as f:
                f.write(html)
            logger.debug(f"Saved article debug HTML for {stub.url}")
        except Exception:
            pass
        # ─────────────────────────────────────────────────────────────────────
